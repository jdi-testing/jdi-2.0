...
import static com.epam.jdi.uitests.gui.sikuli.elements.BaseElement.actionScenarios;
...
public class ActionInvoker {
    ...
    public final <TResult> TResult doJActionResult(String actionName, Supplier<TResult> action,
        ...
        try {
            processDemoMode();
            return actionScenarios.setElement(element).resultScenario(actionName, action, logResult, level);
    ...
    public final void doJAction(String actionName, JAction action, LogLevels level) {
        try {
            processDemoMode();
            actionScenarios.setElement(element).actionScenario(actionName, action, level);
        } catch (Exception | Error ex) {
            throw exception("Failed to do '%s' action. Exception: %s", actionName, ex);
        }
    }
